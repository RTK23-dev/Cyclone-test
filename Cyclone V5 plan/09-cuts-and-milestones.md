...
| **alpha.7** (in progress 2026-09-24) | Mapping depth (Quick / Standard / Deep), type + scroll on the phone from the PC, scenario health per app on Apps, `door-missing` cause | `apps.list` scenario counts; `door-missing` from the Atlas | Choose how deep to map; drive text fields from the PC; spot failing apps at a glance |
