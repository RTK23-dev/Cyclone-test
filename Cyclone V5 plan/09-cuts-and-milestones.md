...
| **alpha.4** | Inspector v2: rooms per step, map vs model steps, redacted frames, route on the map | Run record v2 (room, expected room, decision source, frames) | See where a route broke on the map and jump to fix it |
| **alpha.5** | **Scenarios** lens + **Versions** tab | Scenario store + health; `atlas.versions` | See Instagram's scenarios like Minitap's board; see needs-remap after an app update |
