# Handoff — physical-device test of Cyclone V5 alpha.13 + Glass 1.0.0-alpha.6

**For:** a coding agent running on the owner's Windows PC, phone connected over USB.
**Replaces:** [`HANDOFF-device-test-glass-alpha2.md`](HANDOFF-device-test-glass-alpha2.md) (alpha.8). Same rules, newer builds, more to test.
...
| Piece | Version | New since alpha.8 |
|---|---|---|
| Cyclone Mobile (phone) | `5.0.0-alpha.13.dev1`, versionCode `154` | "Connect this PC?" notification with a six-digit code; rooms + app version per run step; doors stamped with the app version; `scenarios.list`, `atlas.versions`, `knowledge.get`, `atlas.here`, `runs.mark`; mapping passes recorded as runs; Teal Matrix redesign; calmer Trace Field + tools drawer |
| Cyclone One (Windows) | `1.6.0-alpha.13` | Its runtime serves Glass alpha.6 |
| Cyclone Glass (browser) | `1.0.0-alpha.6` | **Devices**, run inspector v2 (rooms, map vs model, route on the map), App tabs **Screens / Scenarios / Versions / Runs**, **Knowledge**, **You are here**, Mark as expected |

## Get the installs

1. From the GitHub prerelease **`v5.0.0-alpha.13.dev1`** download `Cyclone-5.0.0-alpha.13.dev1.apk`,
   `Cyclone-PC-Companion-1.6.0-alpha.13-Setup.exe` and `SHA256SUMS.txt`. Check: `certutil -hashfile <file> SHA256`.
...
adb install -r Cyclone-5.0.0-alpha.13.dev1.apk                                     # must say Success
adb shell dumpsys package com.cyclone.mobile | findstr "versionName versionCode"   # expect 5.0.0-alpha.13.dev1 / 154
```
...
| ID | Test | Pass when |
|---|---|---|
| **D0** | Versions | Phone `5.0.0-alpha.13.dev1` / 154; One `1.6.0-alpha.13`; Glass sidebar `1.0.0-alpha.6` |
...
- Write `Cyclone V5 plan/orchestrators/glass/returns/RETURN-device-test-alpha13.md`: device + PC facts and SHA256 checks; the
...
- Commit it on a new branch `device-test/alpha13` and push. Never push to `main`, `v5/integration` or release branches; never
